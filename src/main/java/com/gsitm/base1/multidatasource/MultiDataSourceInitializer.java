...
class MultiDataSourceInitializer implements ApplicationListener<MultiDataSourceInitializedEvent> {
    ...
    @PostConstruct
    public void init() {
        if (this.applicationContext.getBeanNamesForType(AbstractRoutingDataSource.class, false, false).length > 0) {
    ...
    private void runSchemaScripts() {
        ...
        for (Map.Entry<String, DataSourceSet> entry : targetDataSourceSet.entrySet()) {
            ...
            if (dataSourceProperties.getSchema() != null) {
                for (String schema : dataSourceProperties.getSchema()) {
                    List<Resource> scripts = getScripts(schema, "schema", dataSourceProperties);
                    if (!scripts.isEmpty()) {
                        runScripts(scripts, dataSource, dataSourceProperties);
                        try {
                            this.applicationContext.publishEvent(new MultiDataSourceInitializedEvent(dataSource, dataSourceName, dataSourceProperties));
    ...
    private void runDataScripts(javax.sql.DataSource dataSource, DataSourceProperties dataSourceProperties) {
        if (dataSourceProperties.getData() != null) {
            for (String data : dataSourceProperties.getData()) {
                List<Resource> scripts = getScripts(data, "data", dataSourceProperties);
                runScripts(scripts, dataSource, dataSourceProperties);
            }
        }
    }
    ...
    private void runScripts(List<Resource> resources, DataSource dataSource, DataSourceProperties dataSourceProperties) {
